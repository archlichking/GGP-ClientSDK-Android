...
public class PeopleStepDefinitions extends BasicStepDefinition {
	...
	public void getUserInfoFromCache() {
		if (GreePlatform.getLocalUser() == null) {
			...
			Log.i(TAG, "Logined as user: "
					+ GreePlatform.getLocalUser().getNickname());
		}
	}
		...
		me.loadFriends(Consts.STARTINDEX_1, pageSize, new GreeUserListener() {
			...
			public void onSuccess(int index, int count, GreeUser[] people) {
				...
				if (people != null) {
					Log.i(TAG, "Get " + people.length + " people");
					Log.i(TAG, "Adding people datas");
					((ArrayList<GreeUser>) getBlockRepo().get(PEOPLE_LIST))
							.addAll(Arrays.asList(people));
				}
				notifyStepPass();
			}
	...
	public void verifyFriendNumber(int num) {
		...
		ArrayList<GreeUser> l = (ArrayList<GreeUser>) getBlockRepo().get(
				PEOPLE_LIST);
		assertEquals("friend count", num, l.size());
	}

	private boolean isUserExists(String friendName) {
		ArrayList<GreeUser> friends = (ArrayList<GreeUser>) getBlockRepo().get(
				PEOPLE_LIST);
	...
	public void verifyFriendExists(String friendName) {
		assertEquals("user " + friendName + " in friend list", true,
				isUserExists(friendName));
	}

	@Then("friend list should not have (.+)")
	public void verifyFriendNotExists(String friendName) {
		assertEquals("user " + friendName + " in friend list", false,
				isUserExists(friendName));
	}
	...
	public void checkUserInIgnoreListAsCondition(String isIncludeMark,
			String userId) {
	...
	private void getIgnoreUser(int pageSize) {
		...
		me.loadIgnoredUserIds(Consts.STARTINDEX_1, pageSize,
				new GreeIgnoredUserListener() {
					@Override
					public void onSuccess(int index, int count, String[] list) {
						Log.d(TAG, "Get ignore list success!");
						Log.i(TAG, "Ignore list size: " + count);
						getBlockRepo()
								.put(IGNORE_LIST, new ArrayList<String>());
						if (list != null) {
							((ArrayList<String>) getBlockRepo()
									.get(IGNORE_LIST)).addAll(Arrays
									.asList(list));
							for (int i = 0; i < list.length; i++) {
								Log.i(TAG, "Block user " + (i + 1) + ": "
										+ list[i]);
							}
						} else {
	...
	public void checkUserInIgnoreList(String isIncludeMark, String userId) {
		if (!"INCLUDE".equals(isIncludeMark)
				&& !"NOTINCLUDE".equals(isIncludeMark))
			fail("Not a valid isIncludeMark!");
		boolean isExists = false;
		if (getBlockRepo().get(IGNORE_LIST) == null)
			fail("Did not get the ignore list!");
		ArrayList<String> ignoreUsers = (ArrayList<String>) getBlockRepo().get(
				IGNORE_LIST);
		...
		assertEquals("user in ignore list", transIsIncludeMark(isIncludeMark),
				isExists);
	}
	...
	public void verifyBlockedUser(final String userId) {
		...
		GreePlatform.getLocalUser().isIgnoringUserWithId(userId,
				new GreeIgnoredUserListener() {
					@Override
					public void onSuccess(int index, int count,
							String[] ignoredUsers) {
