 ...
 */
  private String[] getTextIdsSaved() {
    ArrayList<ModeratedText> moderationList =
        (ArrayList<ModeratedText>) getBlockRepo().get(MODERATION_LIST);
  ...
  private int transStatus(String status) {
    if ("CHECKING".equals(status)) return ModeratedText.STATUS_BEING_CHECKED;
    else if ("APPROVED".equals(status)) return ModeratedText.STATUS_RESULT_APPROVED;
    else if ("DELETED".equals(status)) return ModeratedText.STATUS_DELETED;
    else if ("REJECTED".equals(status)) return ModeratedText.STATUS_RESULT_REJECTED;
    else if ("UNKNOWN".equals(status)) return ModeratedText.STATUS_UNKNOWN;
    else fail("Unknow status of the moderation text!");
    return -100;
  }

  @Then("status of text (.+) should be (\\w+)")
  public void verifyModerationText(String text, String status) {
    ArrayList<ModeratedText> list = (ArrayList<ModeratedText>) getBlockRepo().get(MODERATION_LIST);
    if (list == null || list.size() == 0) fail("No moderation text in the list!");
...
//           */
//          getBlockRepo()
//              .put(MODERATION_LIST, new ArrayList<ModeratedText>(Arrays.asList(textInfo)));
//          notifyStepPass();
//        }
//
//        @Override
  ...
  public void checkStatus(String text, String status) {
    ...
    for (ModeratedText m : l) {
      if (text.equals(m.getContent())) {
        _found++;
        assertEquals("status should be " + status + " but is " + String.valueOf(m.getStatus()),
            transStatus(status), m.getStatus());
      }
    }
    assertTrue(_found == 1);
  }
